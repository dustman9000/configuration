...
const (
	...
	ThanosOperatorVersion = "9db10c0aaf777f5ba5e12a0d986448d9e2bc0ac4"
...
const (
	...
	// ObservatoriumVersion is the image version (git sha) of the Observatorium API and is matched to a commit in
	// the rhobs-konflux-obs-api repository.
	// This is generated by mage sync:konflux and should not be modified manually in normal circumstances.
	// To hot patch for individual environments, update the corresponding observatoriumVersion{Env} constant below.
	ObservatoriumVersion = "f761087109f960b59cd5a1ecf6f6912e2041443b"

	observatoriumVersionStage = ObservatoriumVersion
	observatoriumVersionProd  = ObservatoriumVersion
)
...
func DefaultBaseTemplate() TemplateMaps {
	return TemplateMaps{
		Images: ParamMap[string]{
			...
			ObservatoriumAPI:       fmt.Sprintf("%s:%s", ObservatoriumImage, observatoriumVersionStage),
			SyntheticsAPI:          syntheticsApiImage,
			SyntheticsAgent:        syntheticsAgentImage,
			ConfigReloader:         ConfigReloaderImage,
		},
		Versions: ParamMap[string]{
			...
			ObservatoriumAPI:       observatoriumVersionProd,
		},
		LogLevels: ParamMap[string]{
...
var ProductionImages = ParamMap[string]{
	...
	ApiCache:                   memcachedImage,
	MemcachedExporter:          memcachedExporterImage,
	ObservatoriumAPI:           fmt.Sprintf("%s:%s", ObservatoriumImage, observatoriumVersionProd),
